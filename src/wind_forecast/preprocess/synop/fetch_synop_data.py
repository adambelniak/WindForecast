...
def add_hourly_direction(synop_df, auto_station_df):
    ...
    convolved_cosinus = np.convolve(cosinus, np.full(7, 1 / 7), 'valid')
    convolved_cosinus = np.insert(convolved_cosinus, 0, cosinus[0:3])
    convolved_cosinus = np.append(convolved_cosinus, cosinus[-3:])

    convolved_sinus = np.convolve(sinus, np.full(7, 1 / 7), 'valid')
    convolved_sinus = np.insert(convolved_sinus, 0, sinus[0:3])
    convolved_sinus = np.append(convolved_sinus, sinus[-3:])
