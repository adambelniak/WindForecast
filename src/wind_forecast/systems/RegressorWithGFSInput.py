...
class RegressorWithGFSInput(pl.LightningModule):
    ...
    # Forward
    # ----------------------------------------------------------------------------------------------
    def forward(self, x: torch.Tensor, gfs_input: torch.Tensor) -> torch.Tensor:
        return self.model(x.float(), gfs_input.float())
    ...
    def training_step(self, batch: list[torch.Tensor], batch_idx: int) -> dict[str, torch.Tensor]:  # type: ignore
        ...
        """
        inputs, gfs_inputs, targets = batch
