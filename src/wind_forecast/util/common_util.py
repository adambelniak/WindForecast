...
def split_dataset(dataset, val_split=0.2, chunk_length=20, sequence_length=None):
    ...
    wrong_indices = [i for i in train_indexes if i >= len(train_indexes) + len(val_indexes)]
    print(wrong_indices)
    wrong_indices.extend([i for i in val_indexes if i >= len(train_indexes) + len(val_indexes)])
    print(wrong_indices)
