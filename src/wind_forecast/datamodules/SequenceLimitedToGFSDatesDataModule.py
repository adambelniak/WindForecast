...
class SequenceLimitedToGFSDatesDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        if stage in (None, 'fit'):
            dataset = SequenceLimitedToGFSDatesDataset(config=self.config, train=True)
            length = len(dataset)
            self.dataset_train, self.dataset_val = random_split(dataset, [length - (int(length * self.val_split)), int(length * self.val_split)])
        elif stage == 'test':
            self.dataset_test = SequenceLimitedToGFSDatesDataset(config=self.config, train=False)
