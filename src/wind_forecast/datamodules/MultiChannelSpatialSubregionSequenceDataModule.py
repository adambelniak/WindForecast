...
class MultiChannelSpatialSubregionSequenceDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        if stage in (None, 'fit'):
            dataset = MultiChannelSpatialSubregionDataset(config=self.config, list_IDs=self.IDs, train=True, normalize=True, sequence_length=self.config.experiment.sequence_length)
            length = len(dataset)
            self.dataset_train, self.dataset_val = random_split(dataset, [length - (int(length * self.val_split)), int(length * self.val_split)])
        elif stage == 'test':
            self.dataset_test = MultiChannelSpatialSubregionDataset(config=self.config, list_IDs=self.IDs, train=False, sequence_length=self.config.experiment.sequence_length)
