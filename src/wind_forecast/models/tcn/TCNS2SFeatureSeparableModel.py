...
class TCNS2SFeatureSeparableModel(LightningModule):
        ...
        for first_feat in range(0, self.tcn_features - 1):
            for second_feat in range(first_feat + 1, self.tcn_features):
                ...
                for i in range(num_levels):
                    ...
                    in_channels = (8 if config.experiment.with_dates_inputs else 2) if i == 0 else num_channels[i - 1]
    ...
    def forward(self, batch: Dict[str, torch.Tensor], epoch: int, stage=None) -> torch.Tensor:
        ...
        if self.config.experiment.with_dates_inputs:
            ...
            return self.linear_time_distributed(torch.cat([*outputs, *dates_embedding[1]], -1)).squeeze(-1)
        else:
            if self.use_gfs:
